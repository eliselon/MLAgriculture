...
# Entraîner le nouveau modèle glm avec la validation croisée
glm_model_final <- train(label ~ P + K + humidity + rainfall, data = data.train, method = "multinom", trControl = ctrl)

# Prédire
pred_glm <- predict(glm_model_final, newdata = data.test)
...
# Créer un dataframe avec les valeurs d'accuracy
accuracy_data <- data.frame(
  Model = c("Modèle Complet", "Modèle Sélectionné"),
  Accuracy = c(0.87, 0.95)
)

# Créer le graphique avec des tailles augmentées
ggplot(accuracy_data, aes(x = Model, y = Accuracy, fill = Model)) +
  ...
  geom_text(aes(label = round(Accuracy, 2)), 
            ...
            vjust = -0.5, 
            size = 8) +  
  labs(title = "Comparaison de l'Accuracy des Modèles",
       x = "Type de Modèle",
       y = "Accuracy") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold"),   # Titre du graphique
    axis.title.x = element_text(size = 18),                 # Titre de l'axe X
    axis.title.y = element_text(size = 18),                 # Titre de l'axe Y
    legend.title = element_text(size = 15),                 # Titre de la légende
    legend.text = element_text(size = 15)                   # Texte de la légende
  ) +
  scale_fill_manual(values = c("Modèle Complet" = "blue", "Modèle Sélectionné" = "green"))
...
# Prédiction avec le sous-ensemble de culture
pred_glm_filtered <- predict(glm_model_final, newdata = filtered_test_data)
